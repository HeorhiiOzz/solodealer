...
async function notifyReminder(client){
  const settings=await readTelegramSettings();
  if(!settings?.enabled||!settings?.botToken||!settings?.chatId) return {skipped:true};
  const rows=[
    '⏰ <b>Нагадування SoloDealer</b>',
    '',
    `Пора зв’язатися з клієнтом:`,
    `👤 <b>${esc(client.name)||'Клієнт'}</b>`,
    `📱 ${esc(client.contact)||'—'}`,
    client.car_name?`🚘 ${esc(client.car_name)}`:'',
    client.budget?`💰 ${esc(client.budget)}`:'',
    client.payment_type?`💳 ${esc(client.payment_type)}`:'',
    client.notes?`\n📝 ${esc(String(client.notes).slice(-500))}`:''
  ].filter(Boolean);
  return sendTelegramMessage(settings,rows.join('\n'));
}

module.exports={tgCall,sendTelegramMessage,notifyLead,notifyReminder};
